...
async def open_new_chat_command(interaction: discord.Interaction, user: discord.Member):
    ...
    try:
        ...
        existing_channels = [
            ...
            if interaction.user.name in channel.name and user.name in channel.name
        ]
        ...
        class TradeEndView(discord.ui.View):
            ...
            @discord.ui.button(label="거래 종료 요청", style=discord.ButtonStyle.danger, custom_id="trade:end_request")
            async def end_trade(self, interaction_button: discord.Interaction, button: discord.ui.Button):
                await interaction_button.response.send_message("거래 종료 투표를 시작합니다.", ephemeral=True)

                vote_view = ConfirmView(self.channel_to_edit, self.member1, self.member2, self.admin_role, self.closed_category)
                await self.channel_to_edit.send(
                    f"{self.member1.mention} 님과 {self.member2.mention} 님 모두 거래 종료에 동의해야 합니다.",
                    view=vote_view
                )
        ...
        class ConfirmView(discord.ui.View):
            def __init__(self, channel_to_edit, member1, member2, admin_role, closed_category):
                ...
                self.admin_role = admin_role
                self.closed_category = closed_category
                self.agree_members = set()

            @discord.ui.button(label="👍 종료 동의", style=discord.ButtonStyle.success, emoji="👍",custom_id="trade:agree")
            async def agree(self, interaction_button: discord.Interaction, button: discord.ui.Button):
                ...
                self.agree_members.add(interaction_button.user)
                await interaction_button.response.send_message("종료에 동의하셨습니다.", ephemeral=True)

                if len(self.agree_members) == 2:
                    await self.channel_to_edit.send("양측이 거래 종료에 동의했습니다. 거래방을 종료 카테고리로 이동합니다.")
                    overwrites = {
            ...
            @discord.ui.button(label="👎 종료 거부", style=discord.ButtonStyle.danger, emoji="👎",custom_id="trade:disagree")
            async def disagree(self, interaction_button: discord.Interaction, button: discord.ui.Button):
                if interaction_button.user not in [self.member1, self.member2]:
                    await interaction_button.response.send_message("당신은 이 거래의 당사자가 아닙니다.", ephemeral=True)
                    return

                await self.channel_to_edit.send("한 명이 종료를 거부했습니다. 거래 종료가 취소됩니다.")
                self.stop()
